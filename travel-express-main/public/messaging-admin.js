...
async function initializeUser() {
    const checkUserAsync = async () => {
        ...
        if (!user) {
            ...
            const appMain = document.getElementById("appMainAdmin");
            if (appMain) {
                appMain.innerHTML = `
...
window.creerGroupe = async () => {
    ...
    try {
        ...
        // Rafraîchir immédiatement la liste groupes côté admin sans reload page
        currentTab = "groupes";
        if (typeof window.switchTab === "function") {
            window.switchTab("groupes");
        } else {
            await chargerGroupesList();
        }

        // Ouvrir directement le groupe nouvellement créé
        if (group?.id) {
            setTimeout(() => {
                window.ouvrirGroupe(group.id, group.name || name);
            }, 80);
        }
    } catch (err) { 
...
window.creerDiffusion = async () => {
    ...
    try {
        ...
        // Rafraîchir immédiatement la liste diffusions côté admin sans reload page
        currentTab = "diffusions";
        if (typeof window.switchTab === "function") {
            window.switchTab("diffusions");
        } else {
            await chargerDiffusionsList();
        }

        // Ouvrir directement la diffusion nouvellement créée
        if (broadcast?.id) {
            setTimeout(() => {
                window.ouvrirDiffusion(broadcast.id, broadcast.name || name);
            }, 80);
        }
    } catch (err) { 
...
window.supprimerGroupe = async (groupId, groupName) => {
    ...
    try {
        ...
        // ✅ Utiliser /api/groups/{id} (admin + créateur autorisé)
        const res = await fetch(`${API_URL}/groups/${groupId}`, { 
            method: 'DELETE', 
            headers, 
            credentials: 'include' 
